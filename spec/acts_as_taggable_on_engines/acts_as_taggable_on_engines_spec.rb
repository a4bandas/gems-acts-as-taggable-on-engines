...
describe "Acts As Taggable On" do
  ...
  describe "Taggable Method Generation" do
    before(:each) do
      clean_database!
      TaggableModel.tag_types = []
      TaggableModel.acts_as_taggable_on_engines(:tags, :languages, :skills, :needs, :offerings)
      @taggable = TaggableModel.new(:name => "Bob Jones")
    end
  ...
  describe 'Tagging Contexts' do
    it 'should eliminate duplicate tagging contexts ' do
      TaggableModel.acts_as_taggable_on_engines(:skills, :skills)
      TaggableModel.tag_types.freq[:skills].should_not == 3
    end

    it "should not contain embedded/nested arrays" do
      TaggableModel.acts_as_taggable_on_engines([:array], [:array])
    ...
    it "should not raise an error when passed nil" do
      lambda {
        TaggableModel.acts_as_taggable_on_engines()
      }.should_not raise_error
    end

    it "should not raise an error when passed [nil]" do
      lambda {
        TaggableModel.acts_as_taggable_on_engines([nil])
      }.should_not raise_error
    end
  end
  ...
  describe "@@remove_unused_tags" do
    before do
      ...
      @tagging = ActsAsTaggableOnEngines::Tagging.create(:taggable => @taggable, :tag => @tag, :context => 'tags')
    end

    context "if set to true" do
      before do
        ActsAsTaggableOnEngines.remove_unused_tags = true
      end

      it "should remove unused tags after removing taggings" do
        @tagging.destroy
        ActsAsTaggableOnEngines::Tag.find_by_name("awesome").should be_nil
      end
    end

    context "if set to false" do
      before do
        ActsAsTaggableOnEngines.remove_unused_tags = false
      end

      it "should not remove unused tags after removing taggings" do
        @tagging.destroy
        ActsAsTaggableOnEngines::Tag.find_by_name("awesome").should == @tag
      end
    end
  end
